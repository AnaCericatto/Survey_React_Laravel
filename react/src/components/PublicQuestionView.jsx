export default function PublicQuestionView({ question, index, answerChanged }) {
  ...
  return (
    <>
      <fieldset className="mx-4 mt-4 shadow-md bg-white p-4 rounded">
        <div>
        ...
        <div className="mt-3">
          ...
          {question.type === "text" && (
            <div>
              <input
                ...
                onChange={(ev) => answerChanged(ev.target.value)}
                className="mt-1 border border-gray-300 p-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm  rounded-md"
              />
            </div>
          )}
          {question.type === "textarea" && (
            <div>
              <textarea
                onChange={(ev) => answerChanged(ev.target.value)}
                className="mt-1 border p-2 border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm  rounded-md"
              ></textarea>
            </div>
          )}
        </div>
